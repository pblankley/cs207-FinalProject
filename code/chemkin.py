...
#### TODO 
#### PARSER - BOYUAN
#### Fix two failing tests for new input style - BOYUAN
#### Update / proofread Docs for ReactionSet class

#### TODO
#### MAKE SQL - Ryan 
#### Add tests to increase converage to >90% (pytest -v will give info on lines we are missing)
####      and you might need to build the SQL database before that becomes possible (bc reversable reactions)
#### Update /proofread docs for Reaction class and subclasses

# Parser Question (line 257) : can we only support elementary? arent reversible non elementary??
...
class ReversibleReaction(Reaction):
    ...
    def reaction_coef_backward(self, T):
        ...
        delta_H_over_RT = np.dot(v.T, H_RT)
        delta_S_over_R = np.dot(v.T, S_R)
